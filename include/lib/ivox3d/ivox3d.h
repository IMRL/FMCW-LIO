...
class IVox {
   public:
     ...
     * @param points_to_add
     */
    void Add_Points(const PointVector& points_to_add, bool downsample_on);

    /// get nn
    bool Nearest_Search(const PointType& pt, PointType& closest_pt);

    /// get nn with condition
    bool Nearest_Search(const PointType& pt, PointVector& closest_pt, int max_num = 5, double max_range = 5.0);

    /// get nn in cloud
    bool Nearest_Search(const PointVector& cloud, PointVector& closest_cloud);

    /// get nn with distance
    bool Nearest_Search(const PointType& pt, int max_num, PointVector& closest_pt,
                        std::vector<float>& point_distance, double max_range = 5.0);
...
bool IVox<dim, node_type, PointType>::Nearest_Search(const PointType& pt, PointType& closest_pt) {
...
template <int dim, IVoxNodeType node_type, typename PointType>
bool IVox<dim, node_type, PointType>::Nearest_Search(const PointType& pt, PointVector& closest_pt, int max_num,
                                                      double max_range) {
    std::vector<DistPoint> candidates;
...
template <int dim, IVoxNodeType node_type, typename PointType>
bool IVox<dim, node_type, PointType>::Nearest_Search(const PointType& pt, int max_num, PointVector& closest_pt,
                                                     std::vector<float>& point_distance, double max_range) {
    std::vector<DistPoint> candidates;
    candidates.reserve(max_num * nearby_grids_.size());
...
// #define INNER_TIMER
    ...
    for (auto& it : candidates) {
        closest_pt.emplace_back(it.Get());
        point_distance.emplace_back(static_cast<float>(std::sqrt(it.dist)));
    }
    std::sort(point_distance.begin(), point_distance.end());
    return closest_pt.empty() == false;
}
...
template <int dim, IVoxNodeType node_type, typename PointType>
bool IVox<dim, node_type, PointType>::Nearest_Search(const PointVector& cloud, PointVector& closest_cloud) {
    ...
    std::for_each(std::execution::par_unseq, index.begin(), index.end(), [&cloud, &closest_cloud, this](size_t idx) {
        PointType pt;
        if (Nearest_Search(cloud[idx], pt)) {
...
template <int dim, IVoxNodeType node_type, typename PointType>
void IVox<dim, node_type, PointType>::Add_Points(const PointVector& points_to_add, bool downsample_on) {
